...
/* Set to the interrupt number to be used for sending IPIs
 * via IMSIC, leave it 0 to use MSWI instead. */
...
DEFINE_PLATFORM_INTC_MAP({
	{ .hart_id = 0, .target = { .idc_idx = 0 } },  /* Hart 0 -> IDC 0 */
	{ .hart_id = 1, .target = { .idc_idx = 1 } },  /* Hart 1 -> IDC 1 */
	{ .hart_id = 2, .target = { .idc_idx = 2 } },  /* Hart 2 -> IDC 2 */
	{ .hart_id = 3, .target = { .idc_idx = 3 } },  /* Hart 3 -> IDC 3 */
});
#endif
